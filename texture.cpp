...
glTexture::glTexture(char *FileName)
{
	...
	unsigned char *ImgData = SOIL_load_image(FileName, &ImgWidth, &ImgHeight, 0, SOIL_LOAD_RGBA); //LoadBMP(FileName, ImgWidth, ImgHeight, &ImgData);
...
//--------------------------------------------------------------------------------------------
//
glTexture::glTexture(const char *FileName, std::string newName)
{
	//char *ImgData = NULL;
	int ImgWidth;
	int ImgHeight;
	...
	glSamplerParameteri(SamplerId, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	glSamplerParameteri(SamplerId, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glSamplerParameterf(SamplerId, GL_TEXTURE_MAX_ANISOTROPY_EXT, 32.0f);
	if (TextureId)
		sprintf(_msg, "Zaladowano %s jako ID=%d", FileName, TextureId);
	else
		sprintf(_msg, "Nie udalo sie utworzyc tekstury z %s", FileName);
...
void glTexture::SetName(std::string newName)
{
	Name = newName;
}
